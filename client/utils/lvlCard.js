...
module.exports=({member,isVictory})=>{
  return new Promise(async(resolve,reject)=>{
    try{
      ...
      const addedTokens=utils.addTokens(member.user.id,isVictory);
      const{addedExp,addedLevel}=utils.addExp(member.user.id,isVictory);
      ...
      ctx.fillRect(canvas.width-ctx.measureText(`Level ${level}${addedLevel?` + ${addedLevel}`:""}`).width-30,canvas.height-87,canvas.width,canvas.height);
      ctx.fillStyle="white";
      ctx.fillText(`Level ${level}${addedLevel?` + ${addedLevel}`:""}`,canvas.width-10,canvas.height-60);
      ctx.fillStyle=style.profile_background;
      ctx.fillRect(0,canvas.height-50,canvas.width,canvas.height-10);
      ctx.fillStyle=style.profile_progress;
      ctx.fillRect(0,canvas.height-50,Math.round(canvas.width*(xp+addedExp)/neededXp),canvas.height-10);
      ...
      ctx.fillStyle="#21212b";
      ctx.fillText(`${utils.numberWithCommas(xp)} + ${utils.numberWithCommas(addedExp)} / ${utils.numberWithCommas(neededXp)} xp`,canvas.width-10,canvas.height-15);
      ctx.textAlign="left";
      ...
      ctx.fillStyle="#fff8";
      ctx.textAlign="right";
      ctx.fillText(`${utils.numberWithCommas(tokens)} + ${utils.numberWithCommas(addedTokens)}`,canvas.width-165,canvas.height-90);
      ctx.lineWidth=5;
